...
pub fn parse(
    ...
    let item = match assert_single_type(&parsed_item)? {
        CurrentType::Event(ical_event) => {
            let mut uid = None;
            let mut last_modified = Some(Utc::now());
            ...
            for prop in &ical_event.properties {
                match prop.name.as_str() {
                    ...
                    "DTSTART" => {
                        start = try_parse_date_time_from_property(&prop.value);
                        if start.is_none() {
                            full_day = true;
                            start = parse_date_from_property(&prop.value);
                        }
                    }
                    "DTEND" => {
                        end = try_parse_date_time_from_property(&prop.value);
                        if end.is_none() {
                            full_day = true;
                            end = parse_date_from_property(&prop.value);
                        }
                    }
                        ...
                        if let Some(v) = &prop.value {
                            ...
                            for pair in v.split(";") {
                                if let Some((k, v)) = pair.split_once("=") {
                                    rep.push((k.to_string(), v.to_string()));
                                }
                            }
                            repeat = Some(rep)
                        } else {
            ...
            let start = match start {
                Some(v) => v,
                None => return Err(format!("Missing DTSTART for item {}", item_url).into()),
            };

            let end = match end {
                Some(v) => v,
                None => return Err(format!("Missing DTEND for item {}", item_url).into()),
            };

            let last_modified = match last_modified {
                Some(v) => v,
                None => return Err(format!("Missing DTSTAMP for item {}", item_url).into()),
            };
...
fn try_parse_date_time_from_property(value: &Option<String>) -> Option<DateTime<Utc>> {
    value.as_ref().and_then(|s| parse_date_time(s).ok())
}

fn parse_date_time_from_property(value: &Option<String>) -> Option<DateTime<Utc>> {
    match try_parse_date_time_from_property(value) {
        Some(dt) => Some(dt),
        None => {
...
fn parse_date_from_property(value: &Option<String>) -> Option<DateTime<Utc>> {
    value.as_ref().and_then(|s| {
        parse_date_time(&format!("{}T000000Z", s))
            .map_err(|err| {
...
"#;
    ...
    #[test]
    fn test_parse_date() {
        let d = parse_date_from_property(&Some(String::from("19930603")));
        assert!(d.is_some());
    }
}
